...
export function parseAPIError(errorText: string, statusCode: number): ParsedError {
  try {
    ...
    // Log non-JSON error for debugging
...
export function useImageGeneration() {
  ...
  const generate = useCallback(async (prompt: string, options?: GenerateImageOptions) => {
    try {
        ...
        try {
          ...
          if (task_status === 'SUCCEEDED') {
            ...
            console.error('[Image Generation]', {
              taskId: currentTaskId,
              status: 'FAILED',
              error: 'Task marked as FAILED by DashScope'
            });
      ...
      // Log task creation failure (after all retries exhausted)
...
WHY exponential backoff: Prevents overwhelming the API during outages, increases delay between retries (2s → 3s → 4.5s).
...
WHY console logging: Using [Image Generation] and [Retry Attempt] prefixes enables easy filtering in browser DevTools to debug API calls, retries, and failures.

NOTE: Only retries errors where isRetryable=true (network issues, server errors, rate limits).
  Console logs include taskId, prompt (truncated), status, and error details for debugging.
  </action>
  <verify>
- TypeScript compiles: `npx tsc --noEmit` passes
...
// Log errors when displayed to user
useEffect(() => {
  if (error) {
...
WHY suggestion with emoji: Makes error messages feel more helpful and less scary. 💡 emoji indicates a helpful tip.
...
WHY useEffect logging: Logs when error is displayed to user, making it easy to see what the user sees in the console. [UI Error Displayed] prefix for filtering.
...
- Retry logic with exponential backoff (2s → 3s → 4.5s delays)
...
- Comprehensive console logging with prefixed messages for debugging:
  - [API Error] - DashScope API errors with status code and error details
  - [Network Error] - Browser fetch/timeout errors
  - [Retry Attempt] - Retry attempts with attempt number and delay
  - [Image Generation] - Task lifecycle (creation, polling, success, failure)
  - [UI Error Displayed] - Errors shown to users
...
- **Exponential backoff** - 2s → 3s → 4.5s delays prevent API overload
...
- **Console logging with prefixes** - [API Error], [Network Error], [Retry Attempt], [Image Generation], [UI Error Displayed] for easy filtering
